...
public class PortalReadCsvs implements IPortalReadCsvs {
  ...
  private <T> List<T> readFile(String fileName, Class<T> clazz) {
    ...
    try (BufferedReader br = new BufferedReader(
        new InputStreamReader(getResourceAsStream(fileName), StandardCharsets.UTF_8))) {
  ...
  private InputStream getResourceAsStream(String fileName) {
    InputStream inputStream = PortalReadCsvs.class.getClassLoader().getResourceAsStream(fileName);

    if (inputStream == null) {
      String absolutePath = new File(fileName).getAbsolutePath();
      throw new RuntimeException("File not found in path: " + absolutePath);
    }
